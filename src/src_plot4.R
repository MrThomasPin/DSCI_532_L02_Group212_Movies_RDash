...
#read in movies df
df_movies <- read_csv("data/movies_data_clean_new.csv") %>% 
...
make_graph_1 <- function(years=c(1980, 2017), 
  ...
  p3 <- data %>% 
    group_by(year) %>%
    mutate(sum_metric = sum(!!sym(yaxis_to_plot))) %>% 
    ggplot(aes(x=year, y=sum_metric, group=1, text = paste('</br> Year: ', year,
                                                              '</br>Average ', y_label,"(M): ", round(sum_metric, 2)))) +
    scale_x_continuous(breaks = unique(data$year))+
    geom_line(colour = "#cc6600") +
    scale_y_continuous(labels = comma)+
    geom_point()+
